...
def _upload_to_cpa(
    filepath: str, cpa_url: str, cpa_token: str, proxy: Optional[str], thread_id: int
) -> None:
    """注册成功后上传 Token JSON 到 CPA 管理平台"""
    mp = None
    try:
        ...
        if resp.status_code == 200:
            print(f"[线程 {thread_id}] [CPA] Token JSON 已上传到 CPA 管理平台")
        else:
            print(
                f"[线程 {thread_id}] [CPA] 上传失败: {resp.status_code} - {resp.text[:200]}"
            )
    except Exception as e:
        print(f"[线程 {thread_id}] [CPA] 上传异常: {e}")
    finally:
        if mp:
            mp.close()
        ...
        try:
            ...
            if result:
                ...
                if cpa_url and cpa_token:
                    _upload_to_cpa(file_name, cpa_url, cpa_token, proxy, thread_id)
...
def main() -> None:
    ...
    parser.add_argument(
        "--cpa-url",
        default=None,
        help="CPA管理平台上传地址，你的cpa地址，如 http://localhost:8081/v0/management/auth-files",
    )
    parser.add_argument("--cpa-token", default=None, help="CPA管理平台ui管理平台密码")
    ...
    if args.cpa_url and args.cpa_token:
        print(f"[CPA] 已启用CPA上传: {args.cpa_url}")
